...
func (e Entrypointer) Go() error {
	...
	logger := prod.Sugar()
	logger.Infof("Entering entrypointer script...")
...
func (e Entrypointer) prefetchFilesWithLink(logger *zap.SugaredLogger, str1 string) (string, error) {
	...
	if r.MatchString(str1) {
		...
		logger.Infof("%s - %s - %s", key, fileName, bucketName)
		_, err := os.Open(fileName)
		if err == nil {
			logger.Infof("Already fetched key %s.", key)
		...
		if err != nil {
			logger.Infof("Error while fetching from COS %w", err)
			return "", err
		}
		body, err := ioutil.ReadAll(res.Body)
		if err != nil {
			logger.Infof("Error while reading %w", err)
			return "", err
		}
		logger.Infof("fetched:: \n%s\n", body)
		err = ioutil.WriteFile(fileName, body, fs.ModePerm)
		if err != nil {
			logger.Infof("Error while writing file %w", err)
			return "", err
		}
		return fileName, nil
	} else {
...
func (e Entrypointer) readResultsFromDisk() error {
	...
	for _, resultFile := range e.Results {
		...
		resultFilePath := filepath.Join(pipeline.DefaultResultPath, resultFile)
